...
* You may need to login to npmjs initially: `npm login` (don't worry, if you _**do**_ need to login, any other npm commands will let you know e.g. see the example at the end of this section)
...
if you see something like the following, then `npm login` should sort that out:

```
...
npm notice Publishing to https://registry.npmjs.org/ with tag latest and default access
npm error code E404
npm error 404 Not Found - PUT https://registry.npmjs.org/@jimmythedog%2fmmc-server - Not found
npm error 404
npm error 404  The requested resource '@jimmythedog/mmc-server@0.22.2' could not be found or you do not have permission to access it.
npm error 404
...
1. Increment the production release version, and tag the `git` repository, with: `npm version [patch|minor|major]` (this will increment the version in `package.json` e.g. `npm version minor` will increment from `0.19.0` to `0.20.0`, and tag the git repository)
...
1. _**ELSE**_ perform manual steps:
    1. `npm publish` (Note: If you do not specify the tag with `--tag ...`, the package is automatically tagged with `latest`)
